...
(() => {
  ...
  const indexDocs = [
    ...
    {id:'sante',title:'Pollution de l’air et santé',source:'Santé publique France · risques sanitaires',url:'https://www.santepubliquefrance.fr/pollution-et-sante/air',text:'air pollution exposition particules santé risques pollution'},
    {id:'polluants',title:'Qu’est-ce que la pollution de l’air ?',source:'Santé publique France · définition des polluants',url:'https://www.santepubliquefrance.fr/air/quest-ce-que-la-pollution-de-lair',text:'pollution air particules ozone NO2 émissions air intérieur'},
    {id:'europe',title:'Air pollution : panorama européen',source:'Agence européenne pour l’environnement · synthèse',url:'https://www.eea.europa.eu/en/topics/in-depth/air-pollution',text:'qualité air pollution Europe émissions tendances santé'},
    {id:'oms',title:'Pollution de l’air extérieur : effets sur la santé',source:'OMS · fiche de synthèse',url:'https://www.who.int/news-room/fact-sheets/detail/ambient-(outdoor)-air-quality-and-health',text:'pollution air extérieur santé particules exposition'},
    {id:'reseau',title:'Geod’air : le réseau national de mesures',source:'LCSQA / Ineris · données nationales',url:'https://www.geodair.fr/',text:'qualité air France stations données mesures NO2 concentrations statistiques'}
  ];
  const terms = ['air','meilleur','Lille','pollution','qualité'];
  const pairs = [['air Lille',false],['qualité air',true],['air pollution',true],['better air',true],['air intérieur',true]];
  const triples = [['meilleur air Lille',false],['qualité air Lille',true],['air quality Lille',true],['air pollution Lille',true],['indoor air pollution',true]];
  // Explicit teaching lexicon: translations share one feature; an expansion is
  // a hypothesis about intent, not an assertion of perfect synonymy.
  const translations={quality:'qualite',better:'meilleur',indoor:'interieur'};
  const matchTerms=value=>norm(value).split(' ').map(w=>translations[w]||w);
  const matches=(doc,value)=>matchTerms(value).every(w=>norm(doc.text).split(' ').includes(w));
  function df(value,docs=indexDocs) {return docs.filter(d=>matches(d,value)).length;}
  ...
  // The weights are calculated from these ten abridged teaching texts.
  function docScore(doc) {
    const words=norm(doc.text).split(' ');
    const single=terms.reduce((sum,t,i)=>{const tf=words.filter(w=>w===norm(t)).length;return sum+(i>=3?.15:1)*idf(t)*tf/(tf+1);},0);
    const features=new Map();
    for(const [group,extra] of [...pairs,...triples]){
      const key=[...matchTerms(group)].sort().join(' '),weight=extra?.05:.2;
      if(!features.has(key)||features.get(key).weight<weight)features.set(key,{group,weight});
    }
    const bonus=[...features.values()].reduce((sum,{group,weight})=>sum+(matches(doc,group)?weight*idf(group):0),0);
    return single+bonus;
  }
  ...
  class Demo {
    constructor(slide,deck) {
      ...
      // In the ranking demonstration, a text-selection gesture is not an advance.
      let pointerStart=null,dragged=false;
      if(this.type==='ranking'){
        this.surface.addEventListener('pointerdown',e=>{pointerStart={x:e.clientX,y:e.clientY};dragged=false;});
        this.surface.addEventListener('pointermove',e=>{if(pointerStart&&e.buttons&&Math.hypot(e.clientX-pointerStart.x,e.clientY-pointerStart.y)>6)dragged=true;});
        this.surface.addEventListener('pointerup',e=>{if(pointerStart&&Math.hypot(e.clientX-pointerStart.x,e.clientY-pointerStart.y)>6)dragged=true;pointerStart=null;});
        this.surface.addEventListener('pointercancel',()=>{pointerStart=null;dragged=true;});
      }
      this.surface.addEventListener('click',e=>{
        if(e.target.closest('a,input,label,button,select'))return;
        if(this.type==='ranking'){
          const selection=window.getSelection();
          if(dragged||(selection&&!selection.isCollapsed&&(this.surface.contains(selection.anchorNode)||this.surface.contains(selection.focusNode))))return;
        }
        this.deck.moveState(1);
      });
    }
    cancel() {
      this.generation++;
    ...
    render(index,options={}) {
      ...
      this.slide.classList.remove('is-v4-dark');
    ...
    ranking(index) {
      if(index<1){
        ...
        const total=indexDocs.length;
        const rows=items=>items.map(([name])=>`<div class="v4-term"><span>${esc(name)}</span><b data-term="${esc(name)}" data-df="${df(name)}">0/${total}</b></div>`).join('');
        this.surface.innerHTML=`${searchBar(query,true)}<div class="v5-workbench"><div class="v4-console-head"><span>Mini-index · dix notices pédagogiques abrégées</span><span class="v4-console-status">Analyse · 0 / ${total}</span></div><div class="v4-console-grid"><div class="v4-console-column"><h4>Termes et variantes</h4>${rows(terms.map(t=>[t]))}</div><div class="v4-console-column"><h4>Paires de termes</h4>${rows(pairs)}</div><div class="v4-console-column"><h4>Triples et traduction</h4>${rows(triples)}</div></div><div class="v4-console-foot"><span>Présence dans les notices : 0 à ${total}.<br>Les variantes élargissent le vocabulaire testé.</span><span>quality → qualité · better → meilleur<br>indoor → intérieur · sans double comptage.</span></div><div class="v4-progress"><i></i></div></div>`;
        let start;
        const duration=7000,generation=this.generation;
        this.surface.dataset.calculationDuration=String(duration);
        const finish=()=>{this.surface.querySelectorAll('[data-df]').forEach(el=>{el.textContent=el.dataset.df+'/'+total;el.parentElement.classList.remove('is-active');});this.surface.querySelector('.v4-console-status').textContent=`Scores stabilisés · ${total} / ${total}`;this.surface.querySelector('.v4-progress i').style.width='100%';};
        if(!this.animate){finish();return;}
        const tick=now=>{
          if(generation!==this.generation||this.slide.hidden)return;
          // Use one clock: the first rAF timestamp can predate performance.now().
          if(start===undefined)start=now;
          const p=Math.max(0,Math.min(1,(now-start)/duration));
          const examined=Math.max(0,Math.min(total,Math.floor(p*total))),current=indexDocs[Math.min(total-1,examined)];
          this.surface.querySelectorAll('[data-df]').forEach(el=>{el.parentElement.classList.toggle('is-active',p<1&&matches(current,el.dataset.term));el.textContent=df(el.dataset.term,indexDocs.slice(0,examined))+'/'+total;});
          this.surface.querySelector('.v4-console-status').textContent=`Analyse · ${examined} / ${total}`;
          this.surface.querySelector('.v4-progress i').style.width=(p*100)+'%';
          if(p<1)this.frame(tick);else finish();
        };this.frame(tick);
      }else if(index===2){
        this.surface.innerHTML=`${searchBar(query,true)}<ol class="v4-ranked" aria-label="Les cinq premiers résultats sur dix notices">${ranked.slice(0,5).map((d,i)=>`<li class="v4-rank-row v4-enter" style="animation-delay:${i*110}ms"><span class="v4-rank-index">${String(i+1).padStart(2,'0')}</span><div><p class="v4-rank-title">${esc(d.title)}</p><p class="v4-rank-source">${esc(d.source)} · ${link(d.url)}</p></div><span class="v4-score"><small>score</small>${d.score.toFixed(2).replace('.',',')}</span></li>`).join('')}</ol>`;
      }else{
  ...
  function boot(){
    ...
    window.AtlasDemos={demos,indexDocs,ranked,fieldDocs,evalDocs,selections,measures,df,idf,version:5};
  }
